    ...
    <div class="max-w-[1440px] mx-auto px-4 xl:px-[70px] mt-3 mb-6" x-data="{ wholesaleModalOpen: false }">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-5 lg:gap-6">
            ...
            <div x-data="{ mainImage: '{{ $product->cover_image ? Storage::url($product->cover_image) : '' }}' }" class="lg:col-span-5 flex flex-col">
                <div class="bg-white border border-gray-100 relative flex justify-center items-center p-2 rounded w-full lg:w-[85%] mx-auto aspect-square">
                ...
                @if($product->images && $product->images->count() > 0)
                    <div class="relative mt-2 w-full lg:w-[85%] mx-auto" x-data="{
                        ...
                        <div class="relative bg-white border border-gray-100 rounded-lg p-2 shadow-sm">
                            ...
                            <div x-ref="thumbSlider" class="flex gap-2 overflow-x-auto pb-1 scrollbar-hide px-3 snap-x">
                                <button @click="mainImage = '{{ Storage::url($product->cover_image) }}'" 
                                        ...
                                        :class="mainImage === '{{ Storage::url($product->cover_image) }}' ? 'border-[var(--color-trust-blue)]' : 'border-transparent hover:border-gray-200'">
                                ...
                                @foreach($product->images as $image)
                                    <button @click="mainImage = '{{ Storage::url($image->image_path) }}'" 
                                            class="w-14 h-14 shrink-0 bg-gray-50 rounded border-2 overflow-hidden snap-start"
                                            :class="mainImage === '{{ Storage::url($image->image_path) }}' ? 'border-[var(--color-trust-blue)]' : 'border-transparent hover:border-gray-200'">
            ...
            {{-- Product Info --}}
            <div class="lg:col-span-6 lg:-ml-8 flex flex-col">
                <div class="flex-grow flex flex-col">
                    <div>
                        <div class="mb-5">
                            <div class="border-b border-gray-200 pb-3 mb-3">
                                <h1 class="text-[20px] font-normal text-gray-900 leading-tight mb-2">
                                    {{ $product->name }}
                                </h1>
                                ...
                                <tbody>
                                    @foreach($product->specifications as $key => $value)
                                        <tr class="border-b border-gray-100 last:border-b-0">
                                            <th class="w-1/3 px-6 py-4 font-semibold text-gray-700 bg-gray-50 border-r border-gray-100">{{ $key }}</th>
