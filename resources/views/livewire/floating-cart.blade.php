...
?>
<a href="{{ route('cart') }}" class="fixed right-4 top-1/2 -translate-y-1/2 z-40 hidden md:flex flex-col items-center justify-center bg-white w-[84px] rounded-xl shadow-[0_4px_15px_rgba(0,0,0,0.1)] hover:shadow-[0_6px_25px_rgba(0,0,0,0.15)] transition-all group overflow-hidden border border-gray-200">
    <div class="relative w-full flex justify-center py-5 bg-white">
        <div class="relative inline-block animate-cart-dance">
            {{-- Custom Cart Icon with Plus (Exact match from Falaq Food) --}}
            <svg viewBox="0 0 20 21" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true" class="h-10 w-10 text-[#6c757d] group-hover:text-[#159757] transition-colors">
            ...
            @if($count > 0)
                <span class="absolute -top-3 -right-3.5 bg-[#ef4444] text-white text-[12px] font-bold w-[22px] h-[22px] flex items-center justify-center rounded-full leading-none ring-2 ring-white z-10">
                    {{ $count }}
                </span>
            @endif
        </div>
    ...
    <div class="bg-[#159757] w-full text-center py-2 text-[14px] font-bold text-white transition-colors">
