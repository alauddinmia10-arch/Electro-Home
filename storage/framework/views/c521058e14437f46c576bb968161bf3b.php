...
<a href="<?php echo e(route('cart')); ?>" class="fixed right-0 top-1/2 -translate-y-1/2 z-40 hidden md:flex flex-col items-center justify-center bg-white w-[72px] rounded-l-xl shadow-[0_4px_15px_rgba(0,0,0,0.1)] hover:shadow-[0_6px_25px_rgba(0,0,0,0.15)] transition-all group overflow-hidden border border-gray-200 border-r-0" x-data="{ floatCount: <?php echo e($count); ?>, floatSubtotal: <?php echo e($subtotal); ?> }" @cart-updated-optimistic.window="floatCount += $event.detail.qty_change; floatSubtotal += $event.detail.amount" @cart-updated.window="if($event.detail.subtotal !== undefined) { floatCount = $event.detail.count; floatSubtotal = $event.detail.subtotal; }">
    <div class="relative w-full flex justify-center pt-5 pb-1.5 bg-white">
        <div class="relative inline-block animate-cart-dance">
            
            <svg viewBox="0 0 20 21" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true" class="h-8 w-8 text-[#6c757d] group-hover:text-[#159757] transition-colors">
    ...
    <div class="bg-[#159757] w-full text-center py-1.5 text-[12px] font-bold text-white transition-colors">
        <span wire:ignore x-text="'৳' + new Intl.NumberFormat('en-US').format(floatSubtotal)">৳<?php echo e(number_format($subtotal, 0)); ?></span>
    </div>
</a><?php /**PATH C:\Users\Hafeez Hameed\.gemini\antigravity-ide\scratch\ElectroHome.BD\resources\views\livewire/floating-cart.blade.php ENDPATH**/ ?>